...
export class RegimenesComponent {
  private fb = inject(FormBuilder);
  private service = inject(RegimenesService);
  private alert = inject(AlertService);
  public perms = inject(PermissionsService);

  public crud = useCrud<any>(this.service as any, { itemName: 'Régimen laboral' });
  ...
  abrirModal(modalTemplate: TemplateRef<any>, item?: any) {
    ...
    this.crud.setupModal(item?.id_regimen ?? null);
    this.form.reset();
    if (item) {
  ...
  eliminar(item: any) {
    this.crud.deleteItem(item.id_regimen, '¿Eliminar régimen laboral?', 'Se perderá el registro si no tiene personal activo asignado.');
  }
}
